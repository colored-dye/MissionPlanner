...
__declspec(dllexport) void GEC_FN(gec_init_sym_key_conf_auth)(struct gec_sym_key *k, const uint8_t rawkey[GEC_RAW_KEY_LEN]);
void GEC_FN(gec_init_sym_key_conf)(struct gec_sym_key_conf *k, const uint8_t rawkey[GEC_RAW_KEY_LEN]);
__declspec(dllexport) void GEC_FN(gec_key_material_to_2_channels)(struct gec_sym_key *chan1, struct gec_sym_key *chan2
...
__declspec(dllexport) int GEC_FN(gec_encrypt)(struct gec_sym_key *k, const uint8_t pt[GEC_PT_LEN], uint8_t ct[GEC_CT_LEN]);
__declspec(dllexport) int GEC_FN(gec_decrypt)(struct gec_sym_key *k, const uint8_t ct[GEC_CT_LEN], uint8_t pt[GEC_PT_LEN]);
...
__declspec(dllexport) void GEC_FN(gec_hash)(const uint8_t *input, size_t input_len, uint8_t digest[GEC_HASH_LEN]);
